        ...
        if partes[2] == SYMBOL and partes[1] == TIPO_TRADE and partes[5] == "ABIERTA":
            try:
                ...
                if evaluar_tp and cambio >= TAKE_PROFIT:
                    ...
                    registrar_evento(f"LATERAL BTC: TP {SYMBOL} +{round(cambio,2)}%")
                    ...
                    if be_activo and sl_efectivo >= be_price:
                        ...
                        registrar_evento(f"LATERAL BTC: BE {SYMBOL} protegido tras {round(velas_abiert,1)} velas")
                        ...
                        registrar_evento(f"LATERAL BTC: TRAILING_SL {SYMBOL} ${sl_efectivo}")
                        ...
                        registrar_evento(f"LATERAL BTC: SL {SYMBOL} -{STOP_LOSS}%")
...
def evaluar():
    print(f"[FRANCOTIRADOR LATERAL BTC] Evaluando {SYMBOL}...")
    ...
    if RSI_MIN <= rsi <= RSI_MAX:
        ...
        if "✅" in resultado:
            ...
            registrar_evento(f"FRANCOTIRADOR LATERAL BTC: ENTRADA {fill['precio']} RSI:{rsi} | {resultado}")
            enviar_aviso(
                f"⚖️ FRANCOTIRADOR LATERAL BTC\n"
                f"Symbol: {SYMBOL}\n"
        ...
        registrar_evento(f"FRANCOTIRADOR LATERAL BTC: Sin señal. RSI:{rsi}")
