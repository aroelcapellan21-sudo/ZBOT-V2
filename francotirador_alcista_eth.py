        ...
        if partes[2] == SYMBOL and partes[1] == TIPO_TRADE and partes[5] == "ABIERTA":
            try:
                ...
                if cambio >= TAKE_PROFIT:
                    ...
                    registrar_evento(f"ALCISTA ETH: TP {SYMBOL} +{round(cambio,2)}%")
                    ...
                    if be_activo and sl_efectivo >= be_price:
                        ...
                        registrar_evento(f"ALCISTA ETH: BE {SYMBOL} protegido tras {round(velas_abiert,1)} velas")
                        print(f"  🛡️ BE: ${precio_entrada} → ${sl_efectivo} protegido")
                    elif trailing_on:
                        partes[5] = "TRAILING_SL"
                        registrar_sl(precio_entrada, sl_efectivo, monto_op, MONEDA, TIPO_TRADE)
                        enviar_aviso(f"🎯 TRAILING SL {SYMBOL}\nEntrada: ${precio_entrada}\nSalida: ${sl_efectivo}")
                        registrar_evento(f"ALCISTA ETH: TRAILING_SL {SYMBOL} ${sl_efectivo}")
                        print(f"  🎯 TRAILING_SL: ${precio_entrada} → ${sl_efectivo}")
                    else:
                        partes[5] = "SL"
                        registrar_sl(precio_entrada, sl_efectivo, monto_op, MONEDA, TIPO_TRADE)
                        enviar_aviso(f"🛑 SL {SYMBOL}\nEntrada: ${precio_entrada}\nSalida: ${sl_efectivo}\nPerdida: -{STOP_LOSS}%")
                        registrar_evento(f"ALCISTA ETH: SL {SYMBOL} -{STOP_LOSS}%")
...
def evaluar():
    print(f"[FRANCOTIRADOR ALCISTA ETH] Evaluando {SYMBOL}...")
    ...
    if RSI_MIN <= rsi <= RSI_MAX and ema_c > ema_l:
        ...
        if "✅" in resultado:
            ...
            registrar_evento(f"FRANCOTIRADOR ALCISTA ETH: ENTRADA {precio_actual} RSI:{rsi} | {resultado}")
            enviar_aviso(
        ...
        registrar_evento(f"FRANCOTIRADOR ALCISTA ETH: Sin señal. RSI:{rsi}")
