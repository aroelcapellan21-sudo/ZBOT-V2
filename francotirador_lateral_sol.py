        ...
        if partes[2] == SYMBOL and partes[1] == TIPO_TRADE and partes[5] == "ABIERTA":
            try:
                ...
                if evaluar_tp and cambio >= TAKE_PROFIT:
                    ...
                    registrar_evento(f"LATERAL SOL: TP {SYMBOL} +{round(cambio,2)}%")
                    ...
                    if be_activo and sl_efectivo >= be_price:
                        ...
                        registrar_evento(f"LATERAL SOL: BE {SYMBOL} protegido tras {round(velas_abiert,1)} velas")
                        ...
                        registrar_evento(f"LATERAL SOL: TRAILING_SL {SYMBOL} ${sl_efectivo}")
                        ...
                        registrar_evento(f"LATERAL SOL: SL {SYMBOL} -{STOP_LOSS}%")
...
def evaluar():
    print(f"[FRANCOTIRADOR LATERAL SOL] Evaluando {SYMBOL}...")
    ...
    print("  [LATERAL SOL] Fase pausada — sin entradas nuevas.")
    return
    ...
    if RSI_MIN <= rsi <= RSI_MAX:
        ...
        fila_id = reservar_operacion("LATERAL", precio_actual, rsi, monto_op)
        if fila_id is None:
        ...
        if "✅" in resultado:
            ...
            registrar_evento(f"FRANCOTIRADOR LATERAL SOL: ENTRADA {fill['precio']} RSI:{rsi} | {resultado}")
            enviar_aviso(
                f"⚖️ FRANCOTIRADOR LATERAL SOL\n"
                f"Symbol: {SYMBOL}\n"
        ...
        registrar_evento(f"FRANCOTIRADOR LATERAL SOL: Sin señal. RSI:{rsi}")
