...
|---|---|---|
| 16-09 | *(proyecto separado, `~/deriv-test/`)* ¿El precio de Deriv va atrás de un feed más rápido, y alcanza para actuar? | 🔴 No alcanza — el desfase es de **0,3 s**, no de 1-3. Es real (confirma en 0,65 s contra 3,70 s de un control al azar) pero vive dentro del propio pulso de Deriv, que publica **1 tick por segundo**. Además BTC no tiene Rise/Fall y EURUSD no se pudo medir. ✅ Sirvió para validar las velas de 1 min con las que se midió la reversión (prueba 317) |
| 15-09 | *(proyecto separado, `~/deriv-test/`)* EURUSD revierte tras un movimiento brusco (±0,2% en 60 min) | 🟡 Prometedor pero **sin margen — no se opera** — 55,6-56,9% de acierto revirtiendo, 21 de 47 combinaciones pasan la corrección y aguanta los 3 controles. Con el pago real (78,5-78,6%, break-even 56,0%): a 15 min pierde; a 30 min el margen es de **0,9 pp**, el IC95% cruza cero y la 2ª mitad del año ya es negativa (+0,391 → −0,139 USD/op). Falta: semanas de pago por hora |
| 15-09 | ¿Hay ventaja por día de la semana, fin de semana u hora? (pedido de Ariel: buscar patrones repetibles) | 🔴 Descartado — el efecto horario existe pero es menor que la comisión (0,2 pp); ninguna regla de calendario sobrevive a la verificación cruzada entre dos series sin trades en común. Filtrar por día siempre cuesta más en frecuencia de lo que gana por operación |
| 15-09 | *(proyecto separado, `~/deriv-test/`)* Deriv binarias — ¿alguna hora del día da ventaja en EURUSD? | 🔴 Descartado — 0 de 126 celdas pasan la corrección por comparaciones múltiples sobre 1 año de velas de 1 min (363.129). Ni siquiera hace falta descontar el pago: el break-even es 49–59%. Queda corriendo la captura de pagos por hora (`~/deriv-test/`, cron cada 10 min), que no cambia el veredicto |
