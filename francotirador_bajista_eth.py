            ...
            try:
                ...
                if cambio >= TAKE_PROFIT:
                    ...
                    if be_activo and sl_efectivo <= be_price:
                        ...
                        registrar_evento(f"BAJISTA ETH: BE {SYMBOL} protegido tras {round(velas_abiert,1)} velas")
...
def evaluar():
    ...
    if RSI_MIN <= rsi <= RSI_MAX and ema_c < ema_l:
        ...
        if "✅" in resultado:
            ...
            registrar_evento(f"FRANCOTIRADOR BAJISTA ETH: ENTRADA {precio_actual} RSI:{rsi} | {resultado}")
            enviar_aviso(
