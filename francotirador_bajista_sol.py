        ...
        if partes[2] == SYMBOL and partes[1] == TIPO_TRADE and partes[5] == "ABIERTA":
            try:
                ...
                if cambio >= TAKE_PROFIT:
                    ...
                    registrar_evento(f"BAJISTA SOL: TP {SYMBOL} +{round(cambio,2)}%")
                    ...
                    if be_activo and sl_efectivo <= be_price:
                        ...
                        registrar_evento(f"BAJISTA SOL: BE {SYMBOL} protegido tras {round(velas_abiert,1)} velas")
...
def evaluar():
    print(f"[FRANCOTIRADOR BAJISTA SOL] Evaluando {SYMBOL}...")
    ...
    if RSI_MIN <= rsi <= RSI_MAX and ema_c < ema_l:
        ...
        if "✅" in resultado:
            ...
            registrar_evento(f"FRANCOTIRADOR BAJISTA SOL: ENTRADA {precio_actual} RSI:{rsi} | {resultado}")
            enviar_aviso(
                f"📉 FRANCOTIRADOR BAJISTA SOL\n"
                f"Symbol: {SYMBOL}\n"
        ...
        registrar_evento(f"FRANCOTIRADOR BAJISTA SOL: Sin señal. RSI:{rsi}")
