...
from francotirador_alcista_bnb import evaluar as evaluar_alcista
from francotirador_bajista_bnb import evaluar as evaluar_bajista
from francotirador_lateral_bnb import evaluar as evaluar_lateral
...
def dirigir(fase_global=None):
    ...
    print(f"\n{'='*50}")
    print(f"[DIRECTOR BNB] {timestamp}")
    print(f"{'='*50}")

    cierres = fetch_velas(SYMBOL, limite=210)
    if not cierres:
        print("[DIRECTOR BNB] Sin datos de mercado.")
        return
    ...
    registrar_evento(f"DIRECTOR BNB: Fase {fase} | Precio ${precio_actual} | Cambio {cambio_30v}%")

    if fase == "ALCISTA":
        print(f"  ✅ Activando FRANCOTIRADOR ALCISTA BNB")
        evaluar_alcista()
    elif fase == "BAJISTA":
