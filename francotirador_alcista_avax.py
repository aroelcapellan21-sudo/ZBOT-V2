            ...
            try:
                ...
                if evaluar_tp and cambio >= TAKE_PROFIT:
                    ...
                    if be_activo and sl_efectivo >= be_price:
                        ...
                        registrar_evento(f"ALCISTA AVAX: BE {SYMBOL} protegido tras {round(velas_abiert,1)} velas")
...
def evaluar():
    print(f"[FRANCOTIRADOR ALCISTA AVAX] Evaluando {SYMBOL}...")
    ...
    if RSI_MIN <= rsi <= RSI_MAX and ema_c > ema_l:
        ...
        if "✅" in resultado:
            ...
            registrar_evento(f"FRANCOTIRADOR ALCISTA AVAX: ENTRADA {fill['precio']} RSI:{rsi} | {resultado}")
            enviar_aviso(
                f"🚀 FRANCOTIRADOR ALCISTA AVAX\n"
                f"Symbol: {SYMBOL}\n"
            ...
            registrar_evento(f"ALCISTA AVAX: APERTURA FALLIDA {SYMBOL} | {resultado}")
    else:
        print(f"  Sin señal. RSI:{rsi} EMA_C:{ema_c} EMA_L:{ema_l}")
        registrar_evento(f"FRANCOTIRADOR ALCISTA AVAX: Sin señal. RSI:{rsi}")
