        ...
        if partes[2] == SYMBOL and partes[1] == TIPO_TRADE and partes[5] == "ABIERTA":
            try:
                ...
                if evaluar_tp and cambio >= TAKE_PROFIT:
                    ...
                    registrar_evento(f"LATERAL AVAX: TP {SYMBOL} +{round(cambio,2)}%")
                    ...
                    if be_activo and sl_efectivo >= be_price:
                        ...
                        registrar_evento(f"LATERAL AVAX: BE {SYMBOL} protegido tras {round(velas_abiert,1)} velas")
                        ...
                        registrar_evento(f"LATERAL AVAX: TRAILING_SL {SYMBOL} ${sl_efectivo}")
                        ...
                        registrar_evento(f"LATERAL AVAX: SL {SYMBOL} -{STOP_LOSS}%")
...
def evaluar():
    print(f"[FRANCOTIRADOR LATERAL AVAX] Evaluando {SYMBOL}...")
    ...
    if RSI_MIN <= rsi <= RSI_MAX:
        ...
        if "✅" in resultado:
            registrar_operacion("LATERAL", fill["precio"], rsi, monto_op, fill["qty"])
            registrar_evento(f"FRANCOTIRADOR LATERAL AVAX: ENTRADA {fill['precio']} RSI:{rsi} | {resultado}")
            enviar_aviso(
                f"⚖️ FRANCOTIRADOR LATERAL AVAX\n"
                f"Symbol: {SYMBOL}\n"
        ...
        registrar_evento(f"FRANCOTIRADOR LATERAL AVAX: Sin señal. RSI:{rsi}")
