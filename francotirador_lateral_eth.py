        ...
        if partes[2] == SYMBOL and partes[1] == TIPO_TRADE and partes[5] == "ABIERTA":
            try:
                ...
                if evaluar_tp and cambio >= TAKE_PROFIT:
                    ...
                    registrar_evento(f"LATERAL ETH: TP {SYMBOL} +{round(cambio,2)}%")
                    ...
                    if be_activo and sl_efectivo >= be_price:
                        ...
                        registrar_evento(f"LATERAL ETH: BE {SYMBOL} protegido tras {round(velas_abiert,1)} velas")
                        ...
                        registrar_evento(f"LATERAL ETH: TRAILING_SL {SYMBOL} ${sl_efectivo}")
                        ...
                        registrar_evento(f"LATERAL ETH: SL {SYMBOL} -{STOP_LOSS}%")
...
def evaluar():
    print(f"[FRANCOTIRADOR LATERAL ETH] Evaluando {SYMBOL}...")
    ...
    if RSI_MIN <= rsi <= RSI_MAX:
        ...
        if "✅" in resultado:
            ...
            registrar_evento(f"FRANCOTIRADOR LATERAL ETH: ENTRADA {fill['precio']} RSI:{rsi} | {resultado}")
            enviar_aviso(
                f"⚖️ FRANCOTIRADOR LATERAL ETH\n"
                f"Symbol: {SYMBOL}\n"
        ...
        registrar_evento(f"FRANCOTIRADOR LATERAL ETH: Sin señal. RSI:{rsi}")
