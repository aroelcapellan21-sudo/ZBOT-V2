...
def _proteger_otras_fases(fase_activa, precio_actual):
    ...
    entrada".
    No incluye BAJISTA: director_eth.py no importa ese francotirador y hoy no
    pueden existir filas BAJISTA abiertas (gate global gestor_bajistas.bajistas_activos()
    en False bloquea toda entrada bajista).
    """
    for nombre, revisar in (("ALCISTA", cierres_alcista),
        ...
        try:
            ...
            registrar_evento(
                f"DIRECTOR ETH: fallo proteccion cross-fase {nombre}: {e}")

def dirigir(fase_global=None):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"\n{'='*50}")
    print(f"[DIRECTOR ETH] {timestamp}")
    print(f"{'='*50}")

    cierres = fetch_velas(SYMBOL, limite=210)
    if not cierres:
        print("[DIRECTOR ETH] Sin datos de mercado.")
        return
    ...
    registrar_evento(f"DIRECTOR ETH: Fase {fase} | Precio ${precio_actual} | Cambio {cambio_30v}%")
    ...
    if fase == "ALCISTA":
        ...
        registrar_evento(f"DIRECTOR ETH: Fase desconocida. Sin operacion.")
